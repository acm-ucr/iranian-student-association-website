...
const CustomEvents = ({ currentMonth, events }: CustomEventsProps) => {
  ...
  return (
    <div className="">
      {/* calendar days*/}
      <div className="mb-2 grid grid-cols-7 gap-1 text-xs md:text-sm lg:text-base">
        {dayLabels.map((label) => (
      ...
      <div className="grid grid-cols-7 gap-1 lg:gap-2">
        {renderCalendarDays().map((day, index) => {
          ...
          return (
            <div
              key={index}
              className="flex aspect-square flex-col justify-between rounded-lg border border-isa-dark-red bg-white bg-opacity-20 p-1 text-xs sm:text-sm md:text-base lg:text-lg"
            >
              {day && (
                <div className="flex h-full flex-col justify-between">
                  <div className="ml-1 mt-1 text-left font-bold text-isa-dark-red">
                    {day}
                  </div>
                  {dayEvents.length > 0 && (
                    <ul className="mt-1 max-h-full space-y-1 overflow-y-auto">
                      {dayEvents.map((event, idx) => (
                        <li
                          key={idx}
                          className="cursor-pointer overflow-hidden text-ellipsis whitespace-nowrap rounded bg-isa-light-beige p-1 text-xs text-isa-dark-red sm:text-sm"
